...
- (void)drawRect:(CGRect)dirtyRect
{
    if (self.shortcutValue) {
        ...
        NSString *title = (self.recording ? (_hinting ? [MASStrings useOldShorcutButtonTitle] :
                                             (self.shortcutPlaceholder.length > 0 ? self.shortcutPlaceholder :
                                              [MASStrings typeNewShortcutButtonTitle])) : _shortcutValue ? _shortcutValue.description : @"");
        ...
        if (self.recording) {
            ...
            CGRect shortcutRect;
            [self getShortcutRect:&shortcutRect hintRect:NULL];
            NSString *title = (_hinting ? [MASStrings cancelButtonTitle] :
                               (self.shortcutPlaceholder.length > 0 ? self.shortcutPlaceholder : [MASStrings typeShortcutButtonTitle]));
            [self drawInRect:shortcutRect withTitle:title alignment:NSTextAlignmentCenter state:NSControlStateValueOn];
        } else {
            [self drawInRect:self.bounds withTitle:[MASStrings recordButtonTitle] alignment:NSTextAlignmentCenter state:NSControlStateValueOff];
        }
    }
}
                ...
                if (shortcut.keyCodeString.length > 0) {
                    if (!weakSelf.shortcutValidator || [weakSelf.shortcutValidator isShortcutValid:shortcut]) {
                        ...
                        if ([weakSelf.shortcutValidator isShortcutAlreadyTakenBySystem:shortcut explanation:&explanation]) {
                            ...
                            alert.messageText = [MASStrings keyCombinationCannotBeUsedWithTitle:shortcut.description];
                            [alert addButtonWithTitle:[MASStrings okButtonShortcutAlreadyUsed]];
